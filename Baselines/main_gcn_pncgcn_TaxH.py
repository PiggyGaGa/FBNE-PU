...
try:
    with connection.cursor() as cursor:
        ...
        sql = "SELECT XFNSRDZDAH, GFNSRDZDAH from TaxH.VAT_invoice"
        cursor.execute(sql)
...
try:
    with connection.cursor() as cursor:
        # Read a single record
        sql = "SELECT NSRDZDAH FROM TaxH.XYQY"
        cursor.execute(sql)
...
label_file = './label/TaxH_label.pickle'
basic_file = './BasicFeature/Normalize_TaxH.pickle'
...
if __name__ == '__main__':
    ...
    embedding = model.get_embeddings(features, adj, idx2node)
    print(" Optimization Finished ! ")
    embedding.to_pickle('./Result/' + name + 'PnCGCN_embeddings' + str(DIM) + '.pickle')

    # GCN
    name = 'TaxH'
    t_total = time.time()
    adj, features, labels, idx_train, idx_val, idx_test , idx2node = load_nx_adj_data(G, label_file, exist_fea='./feature/TaxH_Normalize.pickle')
    model = GCN(nfeat=features.shape[1], nhid=128, nclass=labels.max().item() + 1, dropout=0.5)
    optimizer = optim.Adam(model.parameters(), lr=lr, weight_decay=weight_decay)
    embedding = model.get_embeddings(features, adj, idx2node)
    for epoch in range(epochs):
    ...
    embedding = model.get_embeddings(features, adj, idx2node)
    print(" Optimization Finished ! ")
    embedding.to_pickle('./Result/' + name + 'GCN_embeddings' + str(DIM) + '.pickle')