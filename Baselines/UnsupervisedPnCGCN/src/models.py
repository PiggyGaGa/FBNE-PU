...
class UnsupervisedLoss(object):
	...
	def get_positive_nodes(self, nodes):
		...
		# return self.deCentrality_random_walks(nodes)
	...
	def deCentrality_random_walks(self, nodes):
		## 去节点中心性的采样，_run_random_walks 是均匀采样邻居，这个是根据邻居节点的度进行采样，度越大的采样概率越低
		for node in nodes:
			if len(self.adj_lists[int(node)]) == 0:
				continue
			cur_pairs = []
			for i in range(self.N_WALKS):  # 进行一个随机游走的步伐
				curr_node = node
				for j in range(self.WALK_LEN):  # 随机游走的长度，也就是几阶邻居
					neighs = self.adj_lists[int(curr_node)]
					neighs_degrees = {single_node: len(self.adj_lists[single_node]) for single_node in neighs}
					next_node = self.choiceDecentrality(neighs_degrees)
	...
	def choiceDecentrality(self, neighs_degrees):
		## 根据邻居节点的度进行采样
		## neighs_degrees 是字典，key 表示节点id, val表示的是度
		## 需要考虑节点的度是否为0， 增强鲁棒性
		reciprocal_degree = {node: recpicrocal_degree for node, recpicrocal_degree in neighs_degrees.items()}
		for key in reciprocal_degree:
			if reciprocal_degree[key] == 0:
				reciprocal_degree = 1  ## 如果度为0，则给出则将度改为1， 这样也增加度为0的节点的采样概率，让特征进行传播

		assert sum(reciprocal_degree.values()) > 0
			# assert 
		normalize_degree = {node : r_degree/sum(reciprocal_degree.values()) for node, r_degree in reciprocal_degree.items()}
		normal_val = normalize_degree.items()
		# 进行轮盘赌算法抽取对应的节点
		sum_left = 0
		interval = defaultdict(dict)
		for node, probability in normalize_degree.items():
			interval[node] = {'left': sum_left, 'right': sum_left + probability}
			sum_left += probability
		rnd_num = random.random()
		for node, inter_v in interval.items():
			if rnd_num > inter_v['left'] and rnd_num <= inter_v['right']:
				return node
...
class SageLayer(nn.Module):
	...
	def forward(self, self_feats, aggregate_feats, neighs=None):
		...
		# combined = torch.sigmoid(self.weight.mm(combined.t())).t()
		return combined
